...
% Jente Willaert - 10032022
% Adapted to only minimize prime gains @ 23-01-2023 
% Fmincon with direct collocation 
clear all; close all; clc;

info.subj = {'CP12'};
info.cond = {'TO'};
info.t    = 4 % Level niveau
info.leg  = 'l';
info.map  = ['C:\Users\u0125183\OneDrive - KU Leuven\Active Reactive Balance\Data/',char(info.subj)];
info.selection =  [1:8 ]
opt       = 'TEST';
...
%% Define stiction based om ankle angle 
new              = IK(1,:)- IK(1,50); 
...
% RMSE
y = EMG(1,11:end-90); yhat  = EMG_recon_MG; %90
RMSE.MG = sqrt(mean((y - yhat).^2));
y = EMG(2,11:end-90); yhat  = EMG_recon_TA;
RMSE.TA = sqrt(mean((y - yhat).^2));
y = EMG(3,11:end-90); yhat  = EMG_recon_SOL;
RMSE.SOL = sqrt(mean((y - yhat).^2));
y = EMG(4,11:end-90); yhat = EMG_recon_LG; 
RMSE.LG  = sqrt(mean((y - yhat).^2)); 
output.RMSE = RMSE;

% Linear model (RMSE / R2)
output.mdl.LG = fitlm(EMG(4,11:end-90),EMG_recon_LG);
output.mdl.MG = fitlm(EMG(1,11:end-90),EMG_recon_MG);
output.mdl.SOL= fitlm(EMG(3,11:end-90),EMG_recon_SOL);
output.mdl.TA = fitlm(EMG(2,11:end-90),EMG_recon_TA);

name = [info.map,'/EMG/EMGreconstruction/TO/output/EMGreconstruction_',char(info.cond),'_Level',num2str(info.t),'_output_',opt,'.mat'];
save(name, 'output')
